	...
	const float bitmap_scaling_factor)
{
	...
	if (error != 0)
	{
		...
		Rml::Log::Message(Rml::Log::LT_WARNING, "Unable to load glyph at index '%u' on the font face '%s %s'; error message: %s.",
			(unsigned int)glyph_index, ft_face->family_name, ft_face->style_name, error_message);
		return false;
	}
	...
	{
		auto error_message = GetFreeTypeErrorString(error);
		Rml::Log::Message(Rml::Log::LT_WARNING, "Unable to render glyph at index '%u' on the font face '%s %s'; error message: %s.",
			(unsigned int)glyph_index, ft_face->family_name, ft_face->style_name, error_message);
		return false;
	}
